...
- 阶段 4 已开始落地：类别面板、标注列表、图片列表、顶部工作台工具栏、数据看板外壳、导出面板、智能过滤面板模板已经组件化；自动标注控制和人工校对工具条已拆到独立组件。类别面板渲染、选择、删除和新增弹窗已迁移到 `ClassController`；自动标注配置控件和 SAM3 API 测试已迁移到 `AutoConfigController`；智能过滤事件绑定、payload 收集、任务轮询和结果摘要已迁移到 `SmartFilterController`，数据看板内容渲染和重建索引事件已迁移到 `DataDashboardController`，导出弹窗事件绑定和导出调用已迁移到 `ExportController`，预览结果渲染、事件绑定和保存逻辑已迁移到 `PreviewController`，单图推理、示例预览、批量任务启动/轮询/暂停恢复、批量配置和结果弹窗已迁移到 `InferenceController`。全局键盘快捷键已迁移到 `KeyboardCommandManager`，GPU 状态轮询和渲染已迁移到 `GpuStatusController`，左右栏和类别/标注/预览区折叠布局已迁移到 `LayoutController`；`image-workspace.js` 页面内 inline `onclick` 已清理；全局任务浮窗也已改为事件委托并对任务文本做 HTML 转义。
...
- 阶段 6 已开始落地：新增 `/project/image/:id/auto` 和 `/project/image/:id/review` 两个入口，旧 `/project/image/:id` 默认进入自动标注入口。`image-workspace-shell.js` 作为共享壳复用同一个 `ImageWorkspace` 和控制器集合，`image-auto-workspace.js` / `image-review-workspace.js` 只传入默认模式；顶部模式切换会同步到对应路由，并在切换前尝试保存脏标注。
- 阶段 7 已开始落地：首批 Pydantic 请求模型已从 `app/main.py` 迁移到 `app/schemas/`，并按 `projects`、`annotations`、`inference`、`filters`、`exports`、`config`、`auth`、`pose` 领域拆分；认证存储服务 `AuthStore` 已迁移到 `app/services/auth_service.py`，认证 cookie/页面辅助已迁移到 `app/services/auth_http.py`，认证页面和认证 API 已迁移到 `app/routers/auth.py`；系统信息、健康检查、全局配置、缓存目录和重启 API 已迁移到 `app/routers/config.py`；项目列表、发现、导入、详情、打开和删除 API 已迁移到 `app/routers/projects.py`；SAM3 health/status、服务管理和 Sapiens checkpoint/status API 已迁移到 `app/routers/services.py`；姿态推理 API 和 Sapiens 结果转 pose annotation 逻辑已迁移到 `app/routers/pose.py`；UI state API 已迁移到 `app/routers/ui_state.py`，UI state 读写持久化已迁移到 `app/repositories/ui_state.py`，项目导出 API 和导出目录解析已迁移到 `app/routers/export.py`；标注读取、保存、追加和追加时 ID 补全逻辑已迁移到 `app/routers/annotations.py`；项目类别更新、追加和删除 API 已迁移到 `app/routers/classes.py`；项目图片列表、统计、索引重建、未标注导航、刷新、导入、上传和删除 API 已迁移到 `app/routers/project_images.py`；数据集上传配置和文件上传 API 已迁移到 `app/routers/uploads.py`；图片原文件访问和 DZI 瓦片访问 API 已迁移到 `app/routers/image_files.py`，瓦片缓存生成/metadata 读取已迁移到 `app/services/image_tiles.py`；单图推理、推理预览、示例预览、批量推理和推理任务控制 API 已迁移到 `app/routers/inference.py`，单图/批量推理执行编排、示例预览、目标图片选择、进度 payload 和暂停任务恢复已迁移到 `app/services/inference_service.py`，推理任务状态、暂停/恢复状态保留和异步执行管理已迁移到 `app/services/inference_jobs.py`，推理检测结果转标注和按类别替换逻辑已迁移到 `app/services/inference_results.py`，视觉提示匹配、正负点/框过滤和自动/人工结果合并逻辑已迁移到 `app/services/inference_visual_prompts.py`；智能过滤同步预览/应用、异步任务控制和过滤运行记录/回滚 API 已迁移到 `app/routers/filters.py`，智能过滤规则归一化、筛选条件判断、项目扫描、标注合并分析、预览缓存和异步任务状态已迁移到 `app/services/smart_filter_service.py`，智能过滤运行记录、快照、latest 查询和回滚持久化已迁移到 `app/repositories/smart_filter_runs.py`；项目 manifest 生成、写入和读取已迁移到 `app/repositories/project_manifests.py`，项目文件层辅助（标注文件路径/计数、legacy annotation 探测、导入重名、项目删除安全清理）已迁移到 `app/repositories/project_files.py`；已拆出的依赖 storage 的 router/service 通过 provider 或回调动态读取当前 storage，避免缓存目录切换后继续使用旧 storage；全局配置读写、缓存目录初始化、环境整数解析和允许数据根目录解析已迁移到 `app/services/config_service.py`；bbox/polygon/mask 转换、IoU、面积和类别族匹配等标注几何工具已迁移到 `app/services/annotation_geometry.py`；ops-api 和 sapiens-api HTTP 请求封装已迁移到 `app/services/integration_clients.py`。API endpoint 函数和响应格式保持不变；旧视频标注 worker、video job schema、SAM3 video session client、video job state 存储入口和未引用的 video export 已删除；`app/main.py` 已降到约 450 行，`app/storage.py` 开始向 repository facade 收敛。

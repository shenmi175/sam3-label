...
- 阶段 7 已开始落地：首批 Pydantic 请求模型已从 `app/main.py` 迁移到 `app/schemas/`，并按 `projects`、`annotations`、`inference`、`filters`、`exports`、`config`、`auth`、`video`、`pose` 领域拆分；认证存储服务 `AuthStore` 已迁移到 `app/services/auth_service.py`。API endpoint 函数和响应格式保持不变。

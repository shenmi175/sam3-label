...
/** Draft emitted by manual-box / manual-polygon creation. */
export interface ManualAnnotationDraft {
  bbox?: ImageBbox;
  polygon?: ImagePoint[];
}
...
type HitResult =
  | { annotation: Annotation; operation: 'move' }
  | { annotation: Annotation; operation: 'bbox-body' }
  | { annotation: Annotation; operation: 'bbox-handle'; handle: HandleName }
  | { annotation: Annotation; operation: 'polygon-vertex'; vertexIndex: number };

interface ClonedGeometry {
  bbox: ImageBbox | null;
  polygon: ImagePoint[] | null;
  polygons: ImagePoint[][] | null;
...
/**
 * TypeScript port of the legacy `ImageViewerV2` canvas component (1:1).
 *
 * Layer stack (inside the container):
...
export class ImageViewerCore {
  ...
  private osdElement: HTMLDivElement;
  ...
  private image: ImageSize | null = null;
  /** Legacy parity: stored but only reset by the core (public in the old JS class). */
  tileInfo: TileInfo | null = null;
  ...
  private staticRenderTimer: number | null = null;
  private staticRenderState: StaticRenderState | null = null;
  private staticExcludeAnnotationId: string | null = null;
  private annotationGeometryCache = new WeakMap<Annotation, GeometryCacheEntry>();
  private wheelFrame: number | null = null;
  private pendingWheelScale = 1;
  ...
  private dragStart: ImagePoint | null = null;
  private dragAnnotation: Annotation | null = null;
  private dragOriginal: ClonedGeometry | null = null;
  private dragMoved = false;
  private dragStartedHistory = false;

  onPromptAdded: PromptAddedHandler | null = null;
  onAnnotationSelected: AnnotationSelectedHandler | null = null;
  ...
  private ensureViewer(): OpenSeadragon.Viewer {
    ...
    const viewportEvents: Array<'animation' | 'animation-finish' | 'pan' | 'zoom' | 'resize'> = [
      'animation', 'animation-finish', 'pan', 'zoom', 'resize',
    ];
  ...
  setImageSource(_tileInfo: TileInfo | null | undefined): boolean {
    return false;
  }

  setPreviewSource(previewInfo: PreviewInfo | null | undefined): boolean {
  ...
  setPrompts(prompts: Prompt[] | null | undefined): void {
  ...
  getPrompts(): Array<{ type: Prompt['type']; data: number[] }> {
    return this.prompts.map((p) => ({ type: p.type, data: p.data }));
  }

  setOptions(nextOptions: ViewerOptions = {}): void {
  ...
  centerOn(bbox: readonly number[] | null | undefined): void {
  ...
  /** Current full-resolution image size in pixels, if an image is loaded. */
  getImageSize(): ImageSize | null {
    return this.image ? { width: this.image.width, height: this.image.height } : null;
  }
  ...
  private requestStaticRedraw(immediate = false): void {
    if (!this.staticCtx) return;
    if (this.staticRenderTimer) {
      window.clearTimeout(this.staticRenderTimer);
      this.staticRenderTimer = null;
    }
    if (!immediate) {
      this.staticRenderTimer = window.setTimeout(() => {
        this.staticRenderTimer = null;
        this.requestStaticRedraw(true);
      }, 90);
      return;
  ...
  private drawStaticAnnotations(): void {
    if (!this.staticCtx) return;
    ...
    if (!this.image || !this.isTileOpen || !this.viewer?.viewport) {
      staticCtx.clearRect(0, 0, this.staticCanvas.width, this.staticCanvas.height);
      this.staticRenderState = null;
      return;
    }
  ...
  private updateStaticTransform(): void {
  ...
  private screenPoint(clientX: number, clientY: number): ImagePoint {
  ...
  screenToImage(clientX: number, clientY: number): ImagePoint {
    ...
    const vp = viewport.pointFromPixel(new OpenSeadragon.Point(x, y), true);
  ...
  imageToScreenRaw(point: readonly number[] | null | undefined): ImagePoint {
    if (!this.viewer?.viewport || !this.image) return [0, 0];
    const viewport = this.viewer.viewport;
    ...
    const px = viewport.pixelFromPoint(vp, true);
    return [px.x, px.y];
  }

  imageToScreen(point: readonly number[] | null | undefined): ImagePoint {
    const raw = this.imageToScreenRaw(point);
    return [raw[0] + this.drawOffsetX, raw[1] + this.drawOffsetY];
  }

  imageScaleRaw(): number {
  ...
  clampPoint(point: readonly number[] | null | undefined): ImagePoint {
  ...
  private normalizeBbox(bbox: readonly number[] | null | undefined): ImageBbox | null {
  ...
  polygonToPairs(points: PolygonInput): ImagePoint[] {
    if (!Array.isArray(points)) return [];
    if (points.length > 0 && typeof points[0] === 'number') {
      const flat = points as readonly number[];
      const out: ImagePoint[] = [];
      for (let i = 0; i < flat.length - 1; i += 2) {
        out.push(this.clampPoint([flat[i], flat[i + 1]]));
      }
      return out;
    }
    return (points as readonly (readonly number[])[])
      .filter((p) => Array.isArray(p) && p.length >= 2)
      .map((p) => this.clampPoint([p[0], p[1]]));
  }

  private pointPairs(points: PolygonInput): ImagePoint[] {
    if (Array.isArray(points) && points.length > 0 && Array.isArray(points[0])) {
      return points as unknown as ImagePoint[];
    }
    return this.polygonToPairs(points);
  }

  private getAnnotationGeometry(ann: Annotation | null | undefined): AnnotationGeometry {
    if (!ann || typeof ann !== 'object') return { bbox: null, polygon: [], polygons: [] };
    const bboxRef: readonly number[] | null = ann.bbox || ann.bbox_xyxy || ann.box || null;
    const polygonRef: PolygonInput = ann.polygon || null;
    const polygonsRef = ann.polygons || null;
    const pointsRef: PolygonInput = ann.points || null;
    ...
    const geometry: AnnotationGeometry = { bbox, polygon, polygons };
  ...
  private invalidateAnnotationGeometry(ann: Annotation | null | undefined): void {
    if (ann && typeof ann === 'object') this.annotationGeometryCache.delete(ann);
  }

  private bboxFromPolygon(points: PolygonInput): ImageBbox | null {
    const pairs = this.polygonToPairs(points);
    return this.bboxFromPairs(pairs);
  }

  private bboxFromPairs(pairs: ImagePoint[]): ImageBbox | null {
  ...
  getAnnotationBbox(ann: Annotation | null | undefined): ImageBbox | null {
    return this.getAnnotationGeometry(ann).bbox;
  }

  private setAnnotationBbox(ann: Annotation, bbox: readonly number[] | null | undefined): void {
  ...
  private annotationVisible(ann: Annotation): boolean {
  ...
  private pointInBbox(point: ImagePoint, bbox: ImageBbox | null): boolean {
    if (!bbox) return false;
    const [x, y] = point;
    return x >= bbox[0] && x <= bbox[2] && y >= bbox[1] && y <= bbox[3];
  }

  private pointInExpandedBbox(point: ImagePoint, bbox: ImageBbox | null, tolerance = 0): boolean {
    if (!bbox) return false;
    const [x, y] = point;
  ...
  private distanceToSegment(point: ImagePoint, a: ImagePoint, b: ImagePoint): number {
    const [px, py] = point;
    const [ax, ay] = a;
    const [bx, by] = b;
    const dx = bx - ax;
  ...
  private pointNearBboxEdge(point: ImagePoint, bbox: ImageBbox | null, tolerance: number): boolean {
    if (!bbox) return false;
    ...
    return ([
      ...
      [[x2, y2], [x1, y2]],
      [[x1, y2], [x1, y1]],
    ] as Array<[ImagePoint, ImagePoint]>).some(([a, b]) => this.distanceToSegment(point, a, b) <= tolerance);
  }

  private pointInPolygon(point: ImagePoint, polygon: PolygonInput): boolean {
  ...
  private pointInAnnotationPolygons(point: ImagePoint, geom: AnnotationGeometry): boolean {
  ...
  distance(a: readonly number[] | null | undefined, b: readonly number[] | null | undefined): number {
    const dx = Number(a?.[0] || 0) - Number(b?.[0] || 0);
    const dy = Number(a?.[1] || 0) - Number(b?.[1] || 0);
    return Math.sqrt(dx * dx + dy * dy);
  }

  private bboxHandles(bbox: ImageBbox | null): BboxHandle[] {
    if (!bbox) return [];
    const [x1, y1, x2, y2] = bbox;
    const cx = (x1 + x2) / 2;
    const cy = (y1 + y2) / 2;
  ...
  hitTestAnnotation(point: ImagePoint): HitResult | null {
    ...
    if (selected) {
      ...
      if (this.pointInBbox(point, bbox)) {
        return { annotation: selected, operation: polygon.length >= 3 ? 'bbox-body' : 'move' };
      }
    }
  ...
  private applyMoveGeometry(ann: Annotation, original: ClonedGeometry, dx: number, dy: number): void {
    ...
    if (originalPoints && originalPoints.length > 0) {
      ...
      delete ann.points;
      if (originalBbox) {
        this.setAnnotationBbox(ann, [
          originalBbox[0] + dx, originalBbox[1] + dy,
          originalBbox[2] + dx, originalBbox[3] + dy,
        ]);
      } else {
        this.setAnnotationBbox(ann, this.bboxFromPolygon(ann.polygon));
      }
      this.invalidateAnnotationGeometry(ann);
      return;
    }
  ...
  private applyBboxResize(ann: Annotation, original: ClonedGeometry, handle: string, point: ImagePoint): void {
  ...
  panByPixels(dx: number, dy: number): void {
    ...
    const viewport = this.viewer.viewport;
    const delta = viewport.deltaPointsFromPixels(new OpenSeadragon.Point(-dx, -dy), true);
  ...
  /**
   * Zoom by a factor, anchored at the given client coordinates.
   * When no coordinates are given, anchors at the center of the canvas.
   */
  zoomBy(scaleChange: number, clientX?: number, clientY?: number): void {
    if (!this.viewer?.viewport) return;
    const viewport = this.viewer.viewport;
    ...
    const [x, y] = this.screenPoint(anchorClientX, anchorClientY);
    const refPoint = viewport.pointFromPixel(new OpenSeadragon.Point(x, y), true);
    viewport.zoomBy(scaleChange, refPoint, false);
    viewport.applyConstraints();
    this.onViewportChanged();
  }
  ...
  private onMouseDown(e: MouseEvent): void {
    ...
    if (e.button === 0 && this.promptMode === 'none') {
  ...
  private onMouseMove(e: MouseEvent): void {
    const dragAnnotation = this.dragAnnotation;
    const dragOperation = this.dragOperation;
    const dragStart = this.dragStart;
    const dragOriginal = this.dragOriginal;
    if (this.isDraggingAnnotation && dragAnnotation && dragOperation && dragStart && dragOriginal) {
      const point = this.screenToImage(e.clientX, e.clientY);
      ...
      if (!this.dragStartedHistory) {
        if (this.onAnnotationEditStart) this.onAnnotationEditStart(dragAnnotation);
        this.dragStartedHistory = true;
      }
      if (dragOperation.operation === 'move') this.applyMoveGeometry(dragAnnotation, dragOriginal, dx, dy);
      else if (dragOperation.operation === 'bbox-handle') this.applyBboxResize(dragAnnotation, dragOriginal, dragOperation.handle, point);
      else if (dragOperation.operation === 'polygon-vertex') this.applyPolygonVertexDrag(dragAnnotation, dragOriginal, dragOperation.vertexIndex, point);
      this.requestDraw();
      return;
    }
    if (this.isPanning) {
  ...
  private centerTransformOnBbox(bbox: readonly number[] | null | undefined): boolean {
  ...
  private drawPath(points: PolygonInput, close = true): ImagePoint[] {
    const ctx = this.ctx;
    if (!ctx) return [];
  ...
  private drawPrompt(p: Prompt): void {
    const ctx = this.ctx;
    if (!ctx) return;
  ...
  private drawHandle(point: ImagePoint, color = '#ffffff'): void {
    const ctx = this.ctx;
    if (!ctx) return;
  ...
  private drawFocusedHandles(): void {
    const ctx = this.ctx;
    if (!ctx) return;
  ...
  private drawActivePolygon(): void {
    const ctx = this.ctx;
    if (!ctx) return;
  ...
  private getColorForClass(className: string | null | undefined): string {
    let hash = 0;
  ...
  private colorWithAlpha(color: string | null | undefined, alpha: number): string {
